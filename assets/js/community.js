...
(function () {
  ...
  function initUploadModal() {
    ...
    function showModalError(msg) {
      var errEl = document.getElementById('commUploadError');
      if (!errEl) {
        errEl = document.createElement('p');
        errEl.id = 'commUploadError';
        errEl.style.cssText = 'color:#C8102E;font-size:0.8rem;margin-top:0.5rem;text-align:center;';
    ...
    function handleFile(file) {
      ...
      if (!ALLOWED.includes(file.type)) {
        showModalError('Solo se aceptan JPG, PNG o WebP.');
        return;
      }
      if (file.size > 10 * 1024 * 1024) {
        showModalError('La foto no puede superar 10 MB.');
        return;
      }
      pendingFile = file;
    ...
    async function submitPhoto() {
      ...
      var fname  = 'photos/' + Date.now() + '-' + Math.random().toString(36).slice(2) + '.' + ext;

      try {
        var upRes = await sb.storage.from(BUCKET).upload(fname, pendingFile, {
          cacheControl: '3600',
          upsert: false,
        ...
        showModalError('Error al enviar. Verificá tu conexión e intentá de nuevo.');
      }
    }
